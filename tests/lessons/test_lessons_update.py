...
def test_get_update_form(client, auth):
	auth.login()
	response = client.get(url_for('lessons.update', id=1))
...
def test_update(client, auth, sqlalchemy_session):
	...
	response = client.post(url_for('lessons.update', id=1), data = {'display_name': display_name, 'lesson_sections': '[]', 'csrf_token': csrf_token})
...
def test_update_invalid_display_name(client, auth, sqlalchemy_session):
	csrf_token = auth.login_return_csrf_token()
	response = client.post(url_for('lessons.update', id=4), data = {'display_name': ' ', 'lesson_sections': '[]', 'csrf_token': csrf_token})
...
def test_update_not_found(client, auth):
	csrf_token = auth.login_return_csrf_token()
	response = client.post(url_for('lessons.update', id=10000), data = {'display_name': ' ', 'lesson_sections': '[]', 'csrf_token': csrf_token})
	assert response.status_code == 404

def test_update_not_authenticated(client):
	response = client.post(url_for('lessons.update', id=1), data = {'display_name': 'test', 'lesson_sections': '[]', 'csrf_token': 'xxx'})
	assert response.status_code == 302
	assert response.headers.get('Location') == 'http://localhost/auth/login'

def test_update_no_csrf_token(client, auth):
	csrf_token = auth.login_return_csrf_token()
	response = client.post(url_for('lessons.update', id=1), data = {'display_name': 'test', 'lesson_sections': '[]'})
	assert response.status_code == 302
	assert response.headers.get('Location') == 'http://localhost/auth/login'
