	...
	render() {
		const lesson_sections = this.state.lesson_sections.map( (lesson_section, index, arrayObj) => 
			<LessonSection
			 ...
			 update_concept_tutorial_url={ (this.props.update_concept_tutorial_url).replace('concept_tutorials/0', 'concept_tutorials/' + lesson_section.id.toString() ) }
			 first_item={index==0}
...
const updateConceptTutorialContainer = document.getElementById('update_concept_tutorial_url');
